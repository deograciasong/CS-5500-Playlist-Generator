 ...
 */
export async function analyzeVibeText(text) {
    ...
    if (apiUrl) {
        try {
            ...
            if (typeof tempo !== 'number')
                tempo = 60 + (energy ?? 0.5) * 140;
            return buildVector({ valence: clamp01(valence ?? 0.5), energy: clamp01(energy ?? 0.5), tempo });
        }
        catch (e) {
            console.warn('Vibe analyzer external API failed, falling back to local heuristic', e?.message ?? e);
    ...
    for (const t of tokens) {
        if (wordLists.positive.includes(t))
            pos++;
        if (wordLists.negative.includes(t))
            neg++;
        if (wordLists.highEnergy.includes(t))
            highE++;
        if (wordLists.lowEnergy.includes(t))
            lowE++;
        if (wordLists.acoustic.includes(t))
            acoustic++;
        if (wordLists.instrumental.includes(t))
            instrumental++;
        if (wordLists.angry.includes(t))
            angry++;
        if (wordLists.tempoFast.includes(t))
            tempoFast++;
        if (wordLists.tempoSlow.includes(t))
            tempoSlow++;
    }
    ...
    const opts = { valence: valence01, energy, tempo };
    if (acoustic > 0)
        opts.acousticness = 0.8;
    if (instrumental > 0)
        opts.instrumentalness = 0.6;
...
function mapPolarityToValence(p) {
    if (typeof p !== 'number')
        return undefined;
    // assume p in -1..1
    return (p + 1) / 2;
...
//# sourceMappingURL=vibe-analyzer.js.map