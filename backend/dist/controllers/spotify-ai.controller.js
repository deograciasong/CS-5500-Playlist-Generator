...
import { rankTracksWithGemini as serviceRankTracksWithGemini, estimateAudioFeaturesWithGemini as serviceEstimateAudioFeaturesWithGemini, generatePlaylistMetaWithGemini as serviceGeneratePlaylistMetaWithGemini, logGeminiDebug as serviceLogGeminiDebug, } from "../services/gemini-playlist.service.js";
...
async function fetchSavedTracks(api, pages) {
    ...
    for (const r of results) {
        if (r.status === "fulfilled") {
            ...
            tracks = tracks.concat(items.map((it) => it.track).filter(Boolean));
        }
    }
    const seen = new Set();
...
export const generatePlaylistFromSpotifyGemini = async (req, res) => {
    ...
    try {
        ...
        userTracks.forEach((t) => { if (t?.id)
            trackById[t.id] = t; });
        const chosen = (rankedIds.length > 0 ? rankedIds : userTracks.map((t) => t.id).slice(0, desired))
            .map((id) => trackById[id])
            .filter(Boolean)
            .slice(0, desired);
