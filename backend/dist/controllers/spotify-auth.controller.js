    ...
    try {
        ...
        if (r.status >= 400 || !isToken(r.data)) {
            ...
            res.status(upstreamStatus).json({
                ...
                message: upstream?.error_description ?? upstream?.error ?? "Spotify token exchange failed",
                upstreamStatus,
            });
            return;
        }
...
function isSpotifyTokenResponse(obj) {
    ...
    try {
        ...
        if (r.status >= 400 || !isSpotifyTokenResponse(r.data) || !r.data.access_token) {
            ...
            return res.status(r.status).json({
                ...
                message: r.data.error_description ?? "Unable to refresh token",
            });
        }
