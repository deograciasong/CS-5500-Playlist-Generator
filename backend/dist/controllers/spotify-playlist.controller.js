 ...
 */
import { SpotifyApiError, createSpotifyApiClient, } from "../lib/spotify-api.js";
...
function handleSpotifyError(res, error) {
...
export const addTracksToPlaylistHandler = async (req, res) => {
...
export const replacePlaylistTracksHandler = async (req, res) => {
    const playlistIdParam = req.params.playlistId;
    const payload = req.body;
    if (!isNonEmptyString(playlistIdParam)) {
        res.status(400).json({ error: "invalid_playlist_id" });
...
export const reorderPlaylistTracksHandler = async (req, res) => {
    const playlistIdParam = req.params.playlistId;
    const payload = req.body;
    if (!isNonEmptyString(playlistIdParam)) {
        res.status(400).json({ error: "invalid_playlist_id" });
    ...
    if (typeof payload?.rangeStart !== "number" ||
