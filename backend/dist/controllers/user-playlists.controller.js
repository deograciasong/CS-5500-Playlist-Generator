...
function normalizeSavedAt(input) {
...
function toResponse(payload) {
    ...
    const inferredGemini = payload.playlist?.isGemini === true ||
...
function extractPlaylistPayload(body) {
...
function extractPlaylistUpdate(body) {
    const playlist = body?.playlist ?? body;
    if (!playlist)
        return null;
    const update = {};
...
export const createSavedPlaylist = async (req, res) => {
    const authReq = req;
    ...
    const coverEmoji = typeof req.body?.coverEmoji === "string" && req.body.coverEmoji.trim().length > 0
...
export const listSavedPlaylists = async (req, res) => {
    const authReq = req;
    try {
...
export const getSavedPlaylistById = async (req, res) => {
    const authReq = req;
    const { id } = req.params;
...
export const updateSavedPlaylist = async (req, res) => {
    const authReq = req;
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) {
        res.status(400).json({ error: "invalid_playlist_id" });
    ...
    try {
        const doc = await SavedPlaylist.findOneAndUpdate({ _id: id, userId: authReq.authUserId }, { $set: update }, { new: true });
        if (!doc) {
            res.status(404).json({ error: "playlist_not_found" });
            return;
        }
...
export const deleteSavedPlaylist = async (req, res) => {
    const authReq = req;
    const { id } = req.params;
    if (!mongoose.isValidObjectId(id)) {
        res.status(400).json({ error: "invalid_playlist_id" });
...
//# sourceMappingURL=user-playlists.controller.js.map