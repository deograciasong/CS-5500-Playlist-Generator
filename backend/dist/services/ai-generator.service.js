...
async function fetchAllSavedTrackIds(accessToken, maxItems = 500) {
    const ids = [];
    ...
    while (url && ids.length < maxItems) {
        ...
        try {
            ...
            if (status === 403) {
                const e = new Error('missing_spotify_scope');
...
export async function generatePlaylistFromSpotify(opts) {
    ...
    if (!savedIds.length) {
        const e = new Error('no_saved_tracks');
    ...
    for (const t of localSaved) {
        if (Array.isArray(t.featureVector) && t.featureVector.length)
            vectors.push(t.featureVector);
    ...
    try {
        ...
        if (savedSample.length) {
            ...
            for (const t of meta.data.tracks || []) {
                outTracks.push({ spotifyId: t.id, title: t.name, artists: (t.artists || []).map((a) => a.name).join(', ') });
            }
        }
    }
    catch (e) {
    ...
    if (userVector) {
        ...
        for (let doc = await cursor.next(); doc != null; doc = await cursor.next()) {
            if (savedIds.includes(String(doc.spotifyTrackId)))
                continue; // skip already owned
            const fv = doc.featureVector;
            if (!Array.isArray(fv))
                continue;
            const s = cosineSimilarity(userVector, fv);
            if (s > 0)
                scored.push({ t: doc, score: s });
        }
        scored.sort((a, b) => b.score - a.score);
        for (const s of scored.slice(0, Math.max(0, (length - outTracks.length)))) {
            outTracks.push({ spotifyId: s.t.spotifyTrackId, title: s.t.name, artists: (s.t.artists || []).map((a) => a.name).join(', ') });
        }
    }
    ...
    if (outTracks.length < length) {
        ...
        try {
            ...
            const recs = (r.data.tracks || []).map((t) => ({ spotifyId: t.id, title: t.name, artists: (t.artists || []).map((a) => a.name).join(', ') }));
            for (const r0 of recs) {
                if (outTracks.length >= length)
                    break;
                if (savedIds.includes(r0.spotifyId))
                    continue;
                if (!outTracks.find((x) => x.spotifyId === r0.spotifyId))
                    outTracks.push(r0);
            }
        }
        catch (e) {
...
export async function savePlaylistToSpotify(_opts) {
