...
function sanitizeMoodValue(raw) {
    ...
    if (trimmed.includes("}"))
        return "";
    if (/["']?mood["']?/i.test(trimmed) && !/:/.test(trimmed))
        return "";
    return trimmed;
}
function extractReasoningFromText(text, parsed) {
    ...
    if (typeof text === "string") {
        ...
        if (reasonJson?.[1])
            return reasonJson[1].trim();
    }
    return null;
}
function resolveMoodValue(candidateText, parsed, fallbackInput) {
...
export async function fetchAvailableModels(apiKey, versions) {
    const now = Date.now();
    ...
    for (const version of versions) {
        ...
        try {
            const res = await axios.get(`${GEMINI_ENDPOINT}/${version}/models`, {
...
export async function detectMoodFromText(userInput, libraryContext) {
    ...
    const buildUrl = (version, model) => `${GEMINI_ENDPOINT}/${version}/models/${model}:generateContent?key=${apiKey}`;
    const prompt = [
    ...
    const versionsToTry = Array.from(new Set([GEMINI_API_VERSION, GEMINI_API_VERSION === "v1" ? "v1beta" : "v1"])).filter(Boolean);
    ...
    const attempted = [];
    const fallbackLabel = text.trim().length > 0 ? text.trim().split(/\s+/).slice(0, 4).join(" ") : "Custom Mood";
    const attemptGenerate = async (version, model) => {
        ...
        try {
            ...
            const moodValue = resolveMoodValue(typeof candidateText === "string" ? candidateText : "", parsed, text);
            const moodClean = cleanMoodValue(moodValue) || fallbackLabel;
            if (!moodClean) {
                lastError = "Gemini returned no mood";
                return null;
            }
            const normalizedMood = moodClean.trim().length <= 3 && fallbackLabel ? fallbackLabel : moodClean.trim();
            const moodLabel = decorateMood(normalizedMood);
            const reasoning = extractReasoningFromText(typeof candidateText === "string" ? candidateText : "", parsed) ||
                parsed?.explanation ||
